...
class LonLonRanch(arcade.Window):
    ...
    def setup(self):
        ...
        self.physics_engine = arcade.PhysicsEngineSimple(self.link_character.player_sprite, self.wall_list)  # Her setter vi fysikkenginene som driver collisions og annet
    ...
    def on_draw(self):
        ...
        for x in range(len(self.characters)):  # Her tegner vi begge characters, og for begge characters, vis inventory skal vises tegn inventory. TODO: Split inventory slik at 1 person kan åpne den om gangen
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.UP or key == arcade.key.W:  ## WASD eller piltast bevegelse
            ...
            try:
                if self.characters[0].InventoryLink.get_inventory_contents()[0].get_type_object() == TypeOfObject.Food:
                    self.characters[0].gain_health(
                        self.characters[0].InventoryLink.get_inventory_contents().pop().get_healing_value())
                    print("Gulrot spist")
                    self.characters[0].InventoryLink.items_in_food_inventory = self.characters[
                                                                                   0].InventoryLink.items_in_food_inventory - 1
            ...
            ## TODO: Kun sjekke splittet food inventory, og ikke hele
            if len(self.characters[0].InventoryLink.inventory_contents) <= 7:
                self.characters[0].InventoryLink.append_to_inventory(SetupObjects.create_carrot())
            ...
            if len(self.characters[0].InventoryLink.inventory_contents) <= 7:
                ...
                self.characters[0].InventoryLink.append_to_inventory(SetupObjects.create_rutabaga())
